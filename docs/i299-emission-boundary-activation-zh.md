...
`RULE_BOUNDARY_HEIGHT = 18_000` 是在 `binary64` 發行曲線下**最後一個**被挖出並被驗證的
區塊。從 18,001 開始，正式曲線改為精確十進位版本（`qlab_devnet::emission_exact`），且
`body.coinbase == coinbase_exact(height)` 成為**有效性規則** —— 金額不對的區塊會被拒絕，
發送方會被記為 peer fault。

18,000 及以下的一切都**按已記錄的樣子**沿用（grandfathered）：epoch 1 的 −4114 區塊留著，
每一個 glibc-vs-exact 的 ±1 都留著，不重算。

它是**高度，不是日期**。按蓋章時實測的節奏（約 47.7 blocks/h）大約在 2026-08-20 到達，
誤差約 ±半天的 PoW 波動。請追 tip，不要追日曆。
...
| tip **遠低於** 18,000 —— 至少留約 2 天餘量 | `curl -s https://explorer.qumbra.org/v1/health.json` |
| 四台主機都可連線且 `fid` 一致 | `OPERATOR.md` §3 跨主機檢查 |
| resume 映像在滾 armed 映像**之前**就已建好並推送 | 見 §3 |

🔴 **若 tip 距 18,000 已不到約 2 天而機隊尚未就緒，請在 lab #299 上要求重新蓋章，不要跟
...
- `crates/qlab-node/src/supply.rs` —— `PINNED_EPOCH_EXPECTED`（15 列，epoch 0..=14）
...
🔴 **先建好並推送 resume 映像。** armed 映像會讓網路在 18,000 停住；如果那一刻 resume 映像
還不存在，finality 就會一直停到它存在為止。

## 4. 滾 armed 映像（在高度 18,000 之前）
...
如果某台沒印出 `halts at 18000`，就是沒拿到新映像 —— 先修好再往下。混合族群在設計上是允許的
（`committee-and-governance` §4），但還在舊映像上的主機會在 18,000 之上繼續挖一條升級後族群
會拒絕的分支。

## 5. 讓網路停在 18,000，然後確認邊界已 finalized

18,000 是 checkpoint cadence 的倍數（8 × 2,250），正是為了讓邊界成為 **finalized** 邊界。
在動任何東西之前：

- 每台主機都報 `final=18000`，且 **`fid` 相同**（`OPERATOR.md` §3）；
...
當 ≥⅔ 的委員會金鑰都在 resume 映像上，finality 就會在 18,000 之上恢復。
...
- epoch 15 跨在邊界上（`17_280..=18_431`），它的 expected 側是分段的：已記錄的邊界前前綴，
  加上邊界之上的精確走法。它應該顯示 `AGREED`，而且 **17,280..=18,000 之內任何被沿用的 ±1
...
- 四台都在 resume 映像上，`fid` 相同，`final` 在 18,000 之上持續前進；
- 四台的 `audit-emission --from 18001` 都 exit 0；
