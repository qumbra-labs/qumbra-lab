...
> **⚠️ These figures are pre-B″ and are NOT the baseline to difference against (noted 2026-07-27).** They were taken at **q40 leaf / q80 interior**; B″ ([#47](https://github.com/qumbra-labs/qumbra-lab/issues/47)) moved the lanes to **q43 / q86** and nothing here was re-measured. Same-command baselines on `main` at the time of D3: **leaf 839.2 KB**, **interior proof 1.65 MB**. Subtracting today's numbers from the figures above yields **+51 KB that belongs to B″, not to whatever you are measuring** — D3 nearly wore that charge, and was spared only because its builder re-measured the baseline instead of trusting this file.
>
