...
- **One issuer process per `issuer.v1`** (a Phase-0 limit, accepted on lab #730). The file is written atomically (tmp + rename), but two wallet processes working the same issuer file are not locked against each other; a second pending rotation is refused, and that is the only guard. Run one issuer process per wallet dir.
- **Minting a runtime asset** (C4b): a registration's seed is the issuer's first note of the asset, and `issuer mint` rides it. A mint returns its row's value to the issuer, so it re-arms itself, and the CLI says so. When an issuer holds no note of the asset (every one spent), `issuer update` seeds a fresh one. The C4b done-when (`a_runtime_asset_is_minted_on_its_seed_and_moves_between_holders`) registers asset 21 at runtime, mints 1,000 on the seed, and has a holder pay another holder 400 in it. After each step every node's supply and its `/v1/attest` document (the explorer's own code over that node) agree.
