...
- **A user wallet.** The spend assembly (`qumbra_faucet::annulet`) is the wallet side of an L2 spend, built here because the faucet needs it first. C2 promotes it into the wallet. *(Corrected in C1, lab #718: this line used to say no user wallet derives an L2-spendable `rkm` yet. That was wrong. The wallet's `rkm = H(nk ‖ D ‖ d)` and `nf = H(nk ‖ ρ)` are lane-for-lane the L2 circuit's, so an existing wallet address already receives spendable L2 notes, and a test now proves it. C1 adds the scan: `qumbra-wallet scan --net annulet`.)* *(C2, lab #720: the assembly now lives in `qlab-l2spend`, shared by the wallet and the faucet, and `qumbra-wallet send --net annulet` sends from an ordinary wallet. It fee-splits first when the wallet holds no exact-tariff fee note. A single transaction moves at most one note of a non-fee asset: two such notes cannot be merged in 2×2, which is recorded as a design-level blocker for the pilot.)*
