...
| Phase 1 supported-device floor and final network-wide depth | ⬜ | Deferred 2026-08-24; [`remote-proving-mobile-device-cloud-plan.md`](remote-proving-mobile-device-cloud-plan.md) records the physical-cloud resumption path; floor hardware/OS, the same two-run protocol, predeclared UX threshold and Larry's depth decision remain owed |
...
| Independent Internet-boundary review of the capacity task-book preparation | ✅ | [`remote-proving-service-review-ledger.md`](remote-proving-service-review-ledger.md) records lab `702456d` + deploy `33afc248`; Claude and Grok both approved with non-blocking residuals |
| Valueless capacity execution and evidence | ✅ | Executed 2026-08-25 on an isolated single-purpose host (deploy topology re-pinned `33afc248`→`bb511a4` mid-run via a reviewed two-line caddy-exec fix, deploy PR #258). Full gate ladder H1–H7 granted, executed and torn down the same day; a fresh-context independent §11 review ruled **concurrency ONE is within the observed envelope** (seven single-worker proofs, prover peak ~11.28 GiB < 16 GiB ceiling, clean cancellation and queue-capacity-1, no stop condition). Raw evidence, authority ledger and both rulings live **off-git per §5** at `qumbra-ops/prover-capacity-20260825T012900Z/`. Two operational findings filed separately: deploy #259 (faucet `trusted_proxy_cidrs`), deploy #261 (ingress `read_body`). Residuals recorded for later: running-cancel of a fully-ramped child not directly measured, server-side per-request timing, direct child-process count |
| Valueless pilot approval | ⬜ | Separate Larry gate; not opened by the capacity ruling above, which authorises nothing. A public/multi-client pilot additionally needs Candidate A (Phases 2–5A) and resolution of the carried topology residual (deploy #261) |
