//! `qumbra_wallet::net::http_get` against real sockets (lab #631).
//!
//! The decoder lives in `qlab-http-framing`; these tests prove the wallet
//! client actually calls it. Pre-fix, `read_response_limited` de-chunked via
//! a literal `"transfer-encoding: chunked"` match and ignored `Content-Length`.
//! This is the client that talks to a server the operator chose.
...
use qumbra_wallet::net::http_get;
