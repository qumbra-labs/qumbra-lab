...
//! module started.
//!
//! ## Deliberately not an HTTP crate
//!
//! This binary's dependency surface is chosen, not accidental (see the
//! crate's `Cargo.toml`). The pool talks to its own node on the same host
//! over one connection per request; that is a page of framing, not a reason
//! to take `ureq`/`hyper` into the process that decides who gets paid.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FramingError::HeadTooLarge { got } => {
                write!(f, "http-framing: head-too-large: {got} bytes with no CRLFCRLF")
            }
            ...
            FramingError::Truncated { want, got } => {
                write!(f, "http-framing: truncated-body: want {want} bytes, got {got}")
            }
...
impl Response {
    ...
    /// average out, and the pool talks only to its own node on the same host.
...
/// in-memory bytes in this module's tests and over a real `tiny_http`
/// server in `tests/node_rpc_framing.rs`. A hand-written fixture alone
...
fn read_delimited<R: Read>(
    ...
    match find_header(headers, "Content-Length") {
        Some(v) => {
            let n = parse_decimal(v).ok_or_else(|| FramingError::BadContentLength {
                got: v.to_string(),
            })?;
            w.take(n)
        }
...
fn read_chunked<R: Read>(w: &mut Wire<R>) -> Result<Vec<u8>, FramingError> {
    ...
    loop {
        ...
        let size = parse_hex(size_field).ok_or_else(|| FramingError::BadChunkSize {
            got: clip(&line),
        })?;
        if size == 0 {
...
mod tests {
    ...
    fn a_short_content_length_body_is_named() {
        ...
        assert_eq!(e.to_string(), "http-framing: truncated-body: want 99 bytes, got 5");
    }
