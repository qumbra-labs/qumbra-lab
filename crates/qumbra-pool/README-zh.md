...
没有行长度上限,而那个超时是**单次读**而不是每连接的(#544);而当节点拒绝组装模板时
——链尖有争议时它**正确地**这么做——矿池会**继续用它手上那份旧模板发任务**,于是矿工
是在为一个注定被孤立的链尖计算,而不是被告知"现在没有工作"(#545)。两者都不威胁链、
也不威胁任何密钥,但两者都是让矿工白烧电的方式。**这个端点"暴露且无防护"的窗口,是一个
没有任何人被告知过的窗口——这使它成为我们能拿到的最便宜的修复时间。**
