...
| 项 | 状态 | 卡在 |
|---|---|---|
| 真正挖出一个块,并读出它的收款人 | ⬜ | 算力和运气;矿池正在跑 |
| 公开 stratum 的连接上限、行长度上限、限速 | ⬜ | **lab #544** |
| 持有模板的陈旧度上限 | ⬜ | **lab #545** |
| 对外公布端点 | ⬜ | 上面两行 —— **不是**第一份 share |

🔴 **端点现在可达,但故意不公布,原因就是这两个 issue。** 公开 stratum 没有连接数上限、
没有行长度上限,而那个超时是**单次读**而不是每连接的(#544)。#545 是两个缺陷,不是一个:
poll 线程把 `replace_template` 已经造好的 job **丢掉了**,矿工只在登录时拿到一份活,之后再
也没有(后续每一份 share 都被 `stale job` 拒绝,连接还在,没有任何说明);而持有的模板没有
陈旧度上限时,节点不可达期间那份登录时的 job 会一直被当成有效。节点在链尖有争议时拒绝
组装是正确的,不在本 issue 范围。两者都不威胁链、也不威胁任何密钥,但两者都是让矿工白
烧电的方式。**这个端点"暴露且无防护"的窗口,是一个没有任何人被告知过的窗口——这使它
成为我们能拿到的最便宜的修复时间。** 在协调者验收软件之前,状态板保持 ⬜。
