...
pub const PINNED_S_ATOMIC_AT_BOUNDARY: Option<u64> = Some(89341051168064);
...
pub const PINNED_COMMITTEE_ACCRUAL_AT_BOUNDARY: Option<u64> = Some(13401157666713);
...
mod tests {
    ...
    /// **Activated 2026-08-10 (lab #299/#303).** The pin now carries the glibc
    /// historical value at the boundary, generated by `qumbra-node emission-pins`
    /// on a `linux/aarch64` host and coordinator-verified against an independent
    /// evaluation (`s_atomic(18001) = 89341051168064`). It is a golden literal, not
    /// a recomputation: recomputing the `f64` walk here would reintroduce the
    /// platform dependence this whole change removes from consensus. On the glibc
    /// fleet the pinned value equals what the historical walk produces (18,001 is
    /// not one of #303's divergence heights), so this is still a no-op there; it
    /// becomes the authority on a non-glibc node.
    #[test]
    fn the_boundary_pin_is_the_activated_glibc_value() {
        assert_eq!(PINNED_S_ATOMIC_AT_BOUNDARY, Some(89341051168064));
        assert_eq!(s_atomic_at_boundary(), 89341051168064);
    }
